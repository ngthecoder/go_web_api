...
func SeedData() error {
	var recipeCount, ingredientCount int

	if err := DB.QueryRow("SELECT COUNT(*) FROM recipes").Scan(&recipeCount); err != nil {
		return fmt.Errorf("failed to check recipes: %w", err)
	}

	if err := DB.QueryRow("SELECT COUNT(*) FROM ingredients").Scan(&ingredientCount); err != nil {
	...
	if err := seedIngredients(); err != nil {
		return fmt.Errorf("failed to seed ingredients: %w", err)
	}
	...
	if err := seedRecipeIngredients(); err != nil {
		return fmt.Errorf("failed to seed recipe ingredients: %w", err)
	}
...
func seedIngredients() error {
	...
	for _, ing := range ingredientsData {
		_, err := DB.Exec(
			"INSERT INTO ingredients (name, category, calories_per_100g, description) VALUES ($1, $2, $3, $4)",
			ing.name, ing.category, ing.calories, ing.description)
		if err != nil {
			return fmt.Errorf("error adding %s to ingredients: %w", ing.name, err)
		}
	}
...
func seedRecipes() error {
	...
	for _, rec := range recipesData {
		_, err := DB.Exec(
			`INSERT INTO recipes (name, category, prep_time_minutes, cook_time_minutes, servings, difficulty, instructions, description) 
			 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
			rec.name, rec.category, rec.prepTime, rec.cookTime, rec.servings, rec.difficulty, rec.instructions, rec.description)
		if err != nil {
			return fmt.Errorf("error adding %s to recipes: %w", rec.name, err)
		}
	}
...
func seedRecipeIngredients() error {
	...
	for _, ri := range recipeIngredientsData {
		...
		if err != nil {
			return fmt.Errorf("error adding recipe_ingredients relationship: %w", err)
		}
	}

	log.Println("Recipe ingredients relationships seeded successfully")
	return nil
}
